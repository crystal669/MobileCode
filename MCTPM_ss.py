...
def huber(y_true, y_pred, delta=1):
    ...
    loss = tf.where(tf.abs(y_true - y_pred) < delta, 0.5 * ((y_true - y_pred) ** 2),
...
def data_process(data):
    ...
    train_value = np.concatenate(
        (onehot_address_encoder, train_data_value[:, cols.index('address') + 1:cols.index('weather')]), axis=1)
    ...
    test_value = np.concatenate(
        (onehot_address_encoder, test_data_value[:, cols.index('address') + 1:cols.index('weather')]), axis=1)
    ...
    print("Train Value Shape:", train_value.shape)
    print("Test Value Shape:", test_value.shape)
    print("Test Data Head:", test_value[:5, :])
    print("Train Data Head:", train_value[:5, :])
...
#
# # plot history
# plt.plot(history.history['out_reg_ti_loss'], label='train')
# plt.plot(history.history['val_out_reg_ti_loss'], label='validate')
# plt.xlabel('epochs')
...
print('Test timespan Ytrue_average:', np.average(abs(inv_Ytrue_test[:, (features + n_address + 1)])))
print('Test timespan Ytest_average:', np.average(abs(inv_Ytrue_test[:, features + n_address + 1])))
print('Test timein Ytrue_average:', np.average(abs(inv_test_pred[:, (features + n_address + 2)])))
print('Test timein Ytest_average:', np.average(abs(inv_test_pred[:, features + n_address + 2])))
