...
def data_process(data):
    ...
    # train_value = train_data_value[:, cols.index('address') + 4:cols.index('weather')]
    # train_value = np.concatenate((train_value, onehot_weather_encoder), axis=1)
    ...
    # test_value = test_data_value[:, cols.index('address') + 4:cols.index('weather')]
    # test_value = np.concatenate((test_value, onehot_weather_encoder), axis=1)
    # test_value = np.concatenate((test_value, test_data_value[:, cols.index('weather') + 1:]), axis=1)
...
def series_to_supervised(data, n_in, n_out, dropnan=True):
    ...
    for i in range(n_in, 0, -1):
        cols.append(df.shift(-i).iloc[:, 13:])
        names += [('var%d(t-%d)' % (j + 1, i)) for j in range(13,n_dcol)]
    # 预测序列（t，t+1,t+n）
    # 将一组数据加入cols列表
    print(len(names))
...
#
scaled_trainX = scaled_trainX.reshape((scaled_trainX.shape[0], time_step, (len(cols) - 5 + n_weather)))
scaled_testX = scaled_testX.reshape((scaled_testX.shape[0], time_step, (len(cols) -5 + n_weather)))
