...
def data_process(data):
    ...
    # train_value = np.delete(train_value, cols.index('weather'), axis=1)
    ...
    # test_value = np.delete(test_value, cols.index('weather'), axis=1)
...
features = train_values.shape[1] - (len(cols) - 1 + n_address)
# 分割训练集、验证集和测试集6:2:2
...
#
# # 在训练和测试数据上进行transform
# scaled_value_train = scaler.transform(train_value)
# scaled_value_test = scaler.transform(test_value)
...
scaled_trainX = scaled_trainX.reshape((scaled_trainX.shape[0], time_step, (len(cols) - 1 + n_address)))
scaled_testX = scaled_testX.reshape((scaled_testX.shape[0], time_step, (len(cols) - 1 + n_address)))
